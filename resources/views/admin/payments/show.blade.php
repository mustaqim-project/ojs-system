...
<div class="ds-page-hdr" data-aos="fade-up">
  <div>
    <x-ui.breadcrumb :items="[['label'=>'Admin'],['label'=>'Payments','href'=>route('admin.payments.index')],['label'=>$payment->invoice_code]]"/>
    <h1 class="ds-page-title">Payment Detail</h1>
  </div>
  <x-status-badge :status="$payment->status" :label="$payment->status_label"/>
</div>
  ...
  <div class="col-12 col-lg-7">
    ...
    <div style="background:linear-gradient(135deg,var(--primary) 0%,#1a3a6b 100%);border-radius:12px;padding:28px;color:#fff;margin-bottom:16px;" class="" data-aos="fade-up" data-aos-delay="100">
      <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:20px;flex-wrap:wrap;gap:10px;">
        ...
        <span style="background:rgba(255,255,255,0.12);color:#fff;padding:5px 14px;border-radius:20px;font-size:12px;font-weight:600;border:1px solid rgba(255,255,255,0.2);">{{ $payment->status_label }}</span>
      </div>
      ...
      <div class="row g-3">
        ...
        <div class="col-sm-4">
          <div style="font-size:11px;opacity:0.6;margin-bottom:3px;">Account No.</div>
          <div style="font-family:monospace;font-size:14px;font-weight:600;">{{ $payment->bank_account }}</div>
        </div>
        <div class="col-sm-4">
          <div style="font-size:11px;opacity:0.6;margin-bottom:3px;">Account Name</div>
    ...
    <div class="ds-card" data-aos="fade-up" data-aos-delay="200" style="margin-bottom:16px;">
      ...
      <div style="padding:20px;">
        ...
        <div style="margin-top:12px;padding:12px;background:var(--info-bg);border-radius:8px;">
          <div style="font-size:11px;font-weight:700;color:var(--info);margin-bottom:4px;">Author's Note:</div>
    ...
    @else
    <div class="ds-card" data-aos="fade-up" data-aos-delay="200" style="margin-bottom:16px;">
      <x-ui.empty-state icon="bi-image" title="No proof uploaded" description="The author has not yet uploaded a payment receipt."/>
    </div>
    @endif
    ...
    <div class="ds-alert {{ $payment->status === 'verified' ? 'ds-alert-success' : 'ds-alert-danger' }}" data-aos="fade-up" data-aos-delay="300" style="margin-bottom:16px;">
      ...
      <div>
        ...
        @if($payment->verifiedBy)
          <div style="font-size:12px;opacity:0.8;margin-top:4px;">By {{ $payment->verifiedBy->name }} · {{ $payment->verified_at?->format('d M Y H:i') }}</div>
        @endif
      </div>
    </div>
    @endif
    ...
    <div class="row g-3" data-aos="fade-up" data-aos-delay="400">
      <div class="col-md-6">
        <div style="background:var(--success-bg);border:1px solid var(--success);border-radius:10px;padding:20px;">
          <h4 style="font-size:13px;font-weight:700;color:var(--success);margin-bottom:12px;"><i class="bi bi-check-circle-fill me-2"></i>Approve Payment</h4>
          <form method="POST" action="{{ route('admin.payments.verify',$payment) }}">
            @csrf
            <x-ui.form-field label="Admin Note (optional)">
              <x-ui.textarea name="admin_notes" rows="2" placeholder="Verification note..."></x-ui.textarea>
            </x-ui.form-field>
            <button type="submit" onclick="return confirm('Verify and mark as PAID?')" class="ds-btn ds-btn-suc w-100 justify-content-center">
              <i class="bi bi-check-circle"></i> Verify & Mark Paid
            </button>
          </form>
        </div>
      </div>
      <div class="col-md-6">
        <div style="background:var(--danger-bg);border:1px solid var(--danger);border-radius:10px;padding:20px;">
          <h4 style="font-size:13px;font-weight:700;color:var(--danger);margin-bottom:12px;"><i class="bi bi-x-circle-fill me-2"></i>Reject Payment</h4>
          <form method="POST" action="{{ route('admin.payments.reject',$payment) }}">
            @csrf
            <x-ui.form-field label="Reason" required>
              <x-ui.textarea name="admin_notes" rows="2" required placeholder="Reason for rejection..."></x-ui.textarea>
            </x-ui.form-field>
            <button type="submit" onclick="return confirm('Reject this payment?')" class="ds-btn ds-btn-danger w-100 justify-content-center">
  ...
  <div class="col-12 col-lg-5">
    <div style="position:sticky;top:80px;display:flex;flex-direction:column;gap:16px;">
      ...
      <div class="ds-card" data-aos="fade-up" data-aos-delay="100">
        <div class="ds-card-hdr"><span class="ds-card-title">Author</span></div>
      ...
      <div class="ds-card" data-aos="fade-up" data-aos-delay="200">
        <div class="ds-card-hdr">
          <span class="ds-card-title">Article</span>
        ...
        <div style="padding:16px 20px;">
          ...
          <x-status-badge :status="$payment->article->status" :label="$payment->article->status_label"/>
        </div>
      </div>

      {{-- Transaction Details --}}
      <div class="ds-card" data-aos="fade-up" data-aos-delay="300">
        <div class="ds-card-hdr"><span class="ds-card-title">Transaction Details</span></div>
        <div>
          @php
          $txDetails = [
            'Invoice' => '<span style="font-family:monospace;font-weight:700;">'.$payment->invoice_code.'</span>',
            'Amount'  => '<strong style="font-size:16px;">Rp '.number_format($payment->amount,0,',','.').'</strong>',
            'Created' => $payment->created_at->format('d M Y H:i'),
          ];
          if($payment->uploaded_at) $txDetails['Uploaded'] = $payment->uploaded_at->format('d M Y H:i');
          if($payment->verified_at) $txDetails['Verified'] = $payment->verified_at->format('d M Y H:i');
          @endphp
      ...
      <div class="ds-card" data-aos="fade-up" data-aos-delay="400">
        <div class="ds-card-hdr"><span class="ds-card-title">Payment Progress</span></div>
        <div style="padding:20px;">
          @php
          $steps = [
            ['key'=>'pending',  'label'=>'Invoice Created', 'sub'=>'Awaiting bank transfer'],
            ['key'=>'uploaded', 'label'=>'Proof Uploaded',  'sub'=>'Author uploaded receipt'],
            ['key'=>'verified', 'label'=>'Verified',        'sub'=>'Admin confirmed payment'],
          ];
          ...
          <div style="display:flex;align-items:flex-start;gap:14px;padding-top:20px;">
            ...
            <div>
              ...
              <div style="font-size:12px;color:var(--text-muted);">Proof not accepted</div>
            </div>
          </div>
          @endif
        </div>
