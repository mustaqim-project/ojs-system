...
<head>
    ...
    <style>
        ...
        @media (max-width: 1023px) {
            ...
            .sidebar.open { transform: translateX(0); box-shadow: 0 20px 25px -5px rgba(0, 0, 0, 0.1), 0 10px 10px -5px rgba(0, 0, 0, 0.04); }
    ...
    <div class="wrapper">
        ...
        <aside class="sidebar" id="sidebar">
            ...
            <div class="sidebar-user-card">
                <div style="width: 36px; height: 36px; background: linear-gradient(135deg, #0ea5e9, #2563eb); border-radius: 8px; display: flex; align-items: center; justify-content: center; color: white; font-weight: bold; font-size: 15px; box-shadow: 0 4px 6px rgba(0,0,0,0.2);">
                    {{ strtoupper(substr(auth()->user()->name, 0, 1)) }}
                </div>
                <div style="overflow: hidden; flex: 1;">
                    <div style="font-size: 13px; font-weight: 600; color: #f8fafc; white-space: nowrap; text-overflow: ellipsis; overflow: hidden; font-family: 'Plus Jakarta Sans', sans-serif;">{{ auth()->user()->name }}</div>
                    <div style="font-size: 11px; color: #94a3b8; font-weight: 500; text-transform: capitalize;">{{ auth()->user()->role }}</div>
                </div>
            </div>
        ...
        <div class="main-content">
            ...
            <header class="topbar">
                <div class="d-flex align-items-center">
                    <button class="btn btn-link d-lg-none text-dark p-0 me-3" onclick="openSB()" aria-label="Open menu">
                        ...
                        <nav aria-label="breadcrumb" style="margin-top:2px;">
                            <ol class="breadcrumb m-0" style="font-size: 12px; font-weight: 500;">
                                @foreach($breadcrumbs as $bc)
                                    @if(!$loop->last)
                                        <li class="breadcrumb-item"><a href="{{ $bc['url'] }}" style="color:var(--text-muted); text-decoration:none;">{{ $bc['label'] }}</a></li>
                                    @else
                                        <li class="breadcrumb-item active" aria-current="page" style="color:var(--primary);">{{ $bc['label'] }}</li>
                                    @endif
                                @endforeach
                            </ol>
                        </nav>
                ...
                <div class="d-flex align-items-center gap-3">
                    ...
                    <div class="dropdown">
                        <a href="#" class="text-secondary position-relative" data-bs-toggle="dropdown" aria-expanded="false" style="font-size: 18px;">
                        ...
                        <ul class="dropdown-menu dropdown-menu-end shadow-sm" style="width: 320px; padding: 0; border-radius: 8px; border: 1px solid var(--border, #e2e8f0);">
                            ...
                            <div style="max-height: 300px; overflow-y: auto;">
                                @forelse(auth()->user()->unreadNotifications->take(5) as $notification)
                                    <li>
                                        <a class="dropdown-item d-flex gap-3 align-items-start" href="{{ route('notifications.read', $notification->id) }}" style="padding: 12px 16px; border-bottom: 1px solid #f1f5f9; white-space: normal;">
                                            <div style="width: 32px; height: 32px; border-radius: 8px; display: flex; align-items: center; justify-content: center; flex-shrink: 0; background: rgba(15, 76, 129, 0.1); color: var(--primary, #0f4c81);">
                    ...
                    <div class="dropdown">
                        <div class="d-flex align-items-center gap-2" data-bs-toggle="dropdown" role="button" aria-expanded="false" style="cursor: pointer;">
                            <div style="width: 32px; height: 32px; border-radius: 8px; background: var(--primary, #0f4c81); display: flex; align-items: center; justify-content: center; color: white; font-weight: 600; font-size: 14px;">
                                {{ strtoupper(substr(auth()->user()->name, 0, 1)) }}
                            </div>
                        </div>
                        <ul class="dropdown-menu dropdown-menu-end shadow-sm" style="border: 1px solid var(--border, #e2e8f0); border-radius: 8px;">
                            <li style="padding: 8px 16px; border-bottom: 1px solid var(--border, #e2e8f0); margin-bottom: 4px;">
            ...
            {{-- PAGE CONTENT --}}
            <main class="page-content animate__animated animate__fadeIn" style="animation-duration: 0.6s; animation-delay: 0.2s; animation-fill-mode: both;">
                @if(session('success'))
